  ...
  return (
    <div className="bg-slate-900/60 border border-white/5 rounded-xl p-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
        {data.map((s) => (
          <div key={s.session} className="bg-slate-950/40 rounded-lg px-3 py-2">
            <p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold">{s.session}</p>
            <p className={`text-sm font-bold ${s.total_pnl >= 0 ? "text-emerald-400" : "text-red-400"}`}>
              {s.total_pnl >= 0 ? "+" : ""}{s.total_pnl.toFixed(2)}
            </p>
            <p className="text-[10px] text-slate-500">{s.win_rate.toFixed(0)}% WR · {s.trades} trades</p>
          </div>
        ))}
      </div>
    </div>
